...
export default function LaboratoriosLst() {
  ...
  return (
    <div className="position-relative">
      {isFetching && <LdsBar />}
      <LaboratoriosHead info={info()}/>
      <LaboratoriosFilter isFetching={isFetching} />
            ...
            <Table striped hover className="mb-1">
              <thead className="sticky-top">
                <tr className="text-nowrap">
                  {camposLaboratorio && camposLaboratorio.filter(el=>el.show).map((el) => {
                    return (
                      <th
                        key={el.field_name}
                        onClick={(e) => {
              ...
              <tbody>
                {data && data?.pages.flatMap(el => el.filas).map((laboratorio) => (
                  <LaboratoriosListItem key={laboratorio.id} laboratorio={laboratorio}/>
                ))}
              </tbody>
            </Table>
            <div className="position-relative">
