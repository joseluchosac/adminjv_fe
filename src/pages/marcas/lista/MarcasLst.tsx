...
import { MarcasFilter } from "./MarcasFilter";
...
export default function MarcasLst() {
  ...
  const setMarcaFilterFormSortTable = useMarcasStore(state => state.setMarcaFilterFormSortTable)
  ...
  return (
    <div className="position-relative">
      {isFetching && <LdsBar />}
      <MarcasHead info={info()}/>
      <MarcasFilter isFetching={isFetching} />
            ...
            <Table striped hover className="mb-1">
              <thead className="sticky-top">
                <tr className="text-nowrap">
                  {camposMarca && camposMarca.filter(el=>el.show).map((el) => {
                    return (
                      <th
                        key={el.field_name}
                        onClick={(e) => {
              ...
              <tbody>
                {data && data?.pages.flatMap(el => el.filas).map((marca) => (
                  <MarcasListItem key={marca.id} marca={marca}/>
                ))}
              </tbody>
            </Table>
            <div className="position-relative">
