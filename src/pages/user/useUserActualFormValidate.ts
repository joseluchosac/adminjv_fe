...
function useUserActualFormValidate(userForm: UserFormT) {
  ...
  useEffect(() => {
    const onValidate = () => {
        ...
        if(!userForm.password.match(passwordValidRegex.current) && userForm.password != ""){
          feedbk.password = "Ingrese al menos 3 caracteres sin espacios"
        }else{
          feedbk.password = ""
        }

        if(userForm.password_repeat !== userForm.password && userForm.password != ""){
...
export default useUserActualFormValidate