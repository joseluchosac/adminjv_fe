...
import { RegisterForm } from "../../../core/types"
...
function useRegisterFormValidate(userForm: RegisterForm) {
  ...
  useEffect(() => {
    const onValidate = () => {
        ...
        if(!userForm.password.match(passwordValidRegex.current) ){
          feedbk.password = "Ingrese al menos 3 caracteres sin espacios"
        }else{
          feedbk.password = ""
        }

        if(userForm.password_repeat !== userForm.password ){
